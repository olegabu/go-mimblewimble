...
func TestSendReceive(t *testing.T) {
	...
	input1, _, err := utils.NewOutput(sg, context, uint64(1), ledger.CoinbaseOutput, asset, OutputUnconfirmed)
	assert.NoError(t, err)
	input2, _, err := utils.NewOutput(sg, context, inputValue-1, ledger.CoinbaseOutput, asset, OutputUnconfirmed)
	assert.NoError(t, err)
...
func TestSendReceiveSingle(t *testing.T) {
	sg, err := NewTestSecretGenerator()
	assert.NoError(t, err)
	context, err := secp256k1.ContextCreate(secp256k1.ContextBoth)
	assert.NoError(t, err)
	...
	input1, _, err := utils.NewOutput(sg, context, inputValue, ledger.CoinbaseOutput, asset, OutputUnconfirmed)
	assert.NoError(t, err)
...
func TestExchange(t *testing.T) {
	...
	sendInput1, _, err := utils.NewOutput(sg, context, uint64(1), ledger.CoinbaseOutput, sendAsset, OutputUnconfirmed)
	assert.NoError(t, err)
	sendInput2, _, err := utils.NewOutput(sg, context, sendInputValue-1, ledger.CoinbaseOutput, sendAsset, OutputUnconfirmed)
	assert.NoError(t, err)
	...
	receiveInput1, _, err := utils.NewOutput(sg, context, uint64(1), ledger.CoinbaseOutput, receiveAsset, OutputUnconfirmed)
	assert.NoError(t, err)
	receiveInput2, _, err := utils.NewOutput(sg, context, receiveInputValue-1, ledger.CoinbaseOutput, receiveAsset, OutputUnconfirmed)
	assert.NoError(t, err)
...
func TestSurjectionOutputs(t *testing.T) {
	...
	sendInput1, _, err := utils.NewOutput(sg, context, 1, ledger.CoinbaseOutput, sendAsset, OutputUnconfirmed)
	assert.NoError(t, err)
	sendInput2, _, err := utils.NewOutput(sg, context, 1, ledger.CoinbaseOutput, sendAsset, OutputUnconfirmed)
	assert.NoError(t, err)
	sendWalletInputs := []SavedOutput{*sendInput1, *sendInput2}

	receiveInput1, _, err := utils.NewOutput(sg, context, 2, ledger.CoinbaseOutput, receiveAsset, OutputUnconfirmed)
	assert.NoError(t, err)
	receiveInput2, _, err := utils.NewOutput(sg, context, 1, ledger.CoinbaseOutput, receiveAsset, OutputUnconfirmed)
	assert.NoError(t, err)
